...
export interface KnownVault {
  ...
  /** Quote coin decimal count — resolved from manifest at discovery time. */
...
function parseOptionU64(raw: unknown): bigint | null {
  ...
  // Treat any other falsy shape as None (including { None: true }).
  return null;
}
...
export function parseVaultCreatedEvent(envelope: VaultCreatedEventEnvelope): KnownVault {
  ...
  return {
    ...
    triggered: false,
    // baseCoinType, quoteCoinType, quoteDecimals are populated at discovery time
    // by looking up the vault's pool_id in the manifest (see index.ts discoverVaultsTick).
  };
}
